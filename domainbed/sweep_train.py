...
if __name__ == "__main__":
    ...
    #### IRM-MMD + recon: mmd_lambda:   100.00 ib_lambda:1.00   irm_lambda:     0.10 val acc: 98.78 +- 0.16 test acc: 79.73 +- 0.64
    #### CEM + recon: mmd_lambda:     1.00 ib_lambda:     0.10   irm_lambda:     1.00 val acc: 99.21 +- 0.18 test acc: 87.04 +- 1.33
    #### (chay lai 1 lan nua, check code xem) IRM +recon: val acc:97.77 +- 0.20  test acc:61.11 +- 1.28 - Nen tang do phan giai mmd_lamda len: 'mmd_lambda': 1000, 'ib_lambda': 0, 'irm_lambda': 0.1}
    #### CEM-MMD + recon: ##10: val acc:99.19 +- 0.19  test acc:89.23 +- 0.78 Corresponding hypers: {'mmd_lambda': 10, 'ib_lambda': 1000, 'irm_lambda': 10
    ...
    #     for ib_lambda in [0]:  ### dang lam cho IRM - 0
    ...
    # hparams["normalize"] = True
    # hparams['irm_penalty_anneal_iters'] = 0
    # hparams['entropy_penalty_anneal_iters'] = 0
    # for mmd_lambda in [1000]:  ##10: val acc:99.19 +- 0.19  test acc:89.23 +- 0.78 Corresponding hypers: {'mmd_lambda': 10, 'ib_lambda': 1000, 'irm_lambda': 10
    #     hparams["mmd_lambda"] = mmd_lambda
    #     for entropy_lambda in [0.1, 1, 10, 100, 1000]: 
    #         hparams["entropy_lambda"] = mmd_lambda
    ...
    # hparams["normalize"] = True
    # hparams['irm_penalty_anneal_iters'] = 0
    # for mmd_lambda in [0.1, 1, 10, 100]:
    #     hparams["mmd_lambda"] = mmd_lambda
    #     for ib_lambda in [0.1, 1, 10, 100]:
    #         hparams["ib_lambda"] = ib_lambda
    #         hparams["class_condition"] = False 
    #         for irm_lambda in [0.1, 1, 10, 100]:
    #             hparams["irm_lambda"] = irm_lambda
    #             test_acc, test_std, val_acc, val_std = run(args, hparams, device)
    #             print("RRRRRRR mmd_lambda:{:9.2f} ib_lambda:{:9.2f}   irm_lambda:{:9.2f} " # 
    #                 "val acc: {:.2f} +- {:.2f} test acc: {:.2f} +- {:.2f}".format(
